...
<img width="1901" height="927" alt="image" src="https://github.com/user-attachments/assets/e2b3fb40-9c3a-44d6-b456-7edd8b220273" />
...
| Route | Auth | |
|---|---|---|
| `GET /` | None | Landing, redirects to `/card` if logged in |
| `GET /login` | None | Kicks off GitHub OAuth via Auth0 |
| `GET /callback` | None | Auth0 redirect target |
| `GET /logout` | ✓ | Clears session |
| `GET /search?username=x` | None | Public search, redirects to `/u/:username` |
| `GET /card` | ✓ | Your card, auto-generated from session |
| `GET /u/:username` | None | Public shareable card, no login required |
| `GET /healthz` | None | JSON status + uptime (keep-alive target) |
| `GET /stats` | None | Global usage counters (total cards generated, unique developers) |
